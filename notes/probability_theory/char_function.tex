...
\begin{example}
    ...
    \begin{equation*}
        \varphi_X(t) = \sum_{k=0}^\infty e^{itk}e^{-\lambda}\frac{\lambda^k}{k!} 
        = e^{-\lambda}\sum_{k=0}^\infty \frac{(e^{it}\lambda)^k}{k!} = \exp\pth{\lambda\pth{e^{it}-1}}. 
    \end{equation*}
\end{example}
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \leq 2\P\pth{\abs{X_\alpha}\geq M} + 2M\abs{t} \leq 2(\epsilon + M\abs{t}).  
    \end{equation*}
...
\begin{theorem}\label{thm:moment_char}
...
\end{proof}
\begin{remark}
    The converse does not hold in general. We provide an example 
    where $\varphi(x)$ is differentiable at $0$ but $\E\sbrc{\abs{X}} = \infty$. 
    Consider the random variable defined by $\P(X = \pm k) = \frac{c}{k^2\log(k)}$ 
    for $k\geq 2$ where $c$ is some normalizing constant. Then 
    \begin{equation*}
        \E\sbrc{\abs{X}} = 2c\sum_{k=2}^\infty\frac{1}{k\log(k)}.
    \end{equation*}
    Since the integral 
    \begin{equation*}
        \int_2^\infty \frac{1}{x\log(x)}dx = \eval{\log(\log(x))}{2}{\infty} = \infty,
    \end{equation*}
    the first moment is not finite. However, 
    \begin{equation*}
        \varphi(t) = 2c\sum_{k=2}^\infty \frac{\cos(kt)}{k^2\log(k)}, 
        \quad\text{and}\quad 
        \varphi'(0) = 2c\sum_{k=2}^\infty \frac{0}{k\log(k)} = 0. 
    \end{equation*}
\end{remark}

\begin{corollary}[Taylor Expansion]\label{cor:char_taylor}
    Let $\varphi$ be a characteristic function of a random 
    variable $X$ with $\E\sbrc{\abs{X}^n}<\infty$. Then as $t\to 0$, 
    \begin{equation*}
        \varphi(t) = \sum_{k=0}^n \frac{(it)^k\E\sbrc{X^k}}{k!} + o(t^n).
    \end{equation*} 
\end{corollary}
\begin{proof}
    By \cref{thm:moment_char}, we can consider the Taylor expansion 
    of $\varphi$ up to order $n$ at $t = 0$. 
    \begin{equation*}
        \varphi(t) = \sum_{k=0}^n\frac{\varphi^{(k)}(0)t^k}{k!} + o(t^n) 
        = \sum_{k=0}^n\frac{\E\sbrc{(iX)^k}t^k}{k!} + o(t^n)  
        = \sum_{k=0}^n\frac{(it)^k\E\sbrc{X^k}}{k!} + o(t^n)
    \end{equation*}
    as $t\to 0$. 
\end{proof}
