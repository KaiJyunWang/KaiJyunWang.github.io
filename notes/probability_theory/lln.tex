...
\begin{corollary}\label{cor:conv_in_prob_conv_thms}
    ...
    \begin{thmenum}
        ...
        \item If $X_n\in\L^1$ with $X_n\geq 0$, then $\E\sbrc{X}\leq \liminf_{n\to\infty}\E\sbrc{X_n}$. 
    \end{thmenum}
\end{corollary}
\begin{proof}
    ...
    For (c), note that there is a subsequence $X_{n_k}$ such that $\E\sbrc{X_{n_k}}\to 
    \liminf_{n\to\infty}\E\sbrc{X_n}$. Now, by \cref{thm:prob_conv_subseq_as}, 
    there is a further subsequence of $X_{n_k}$ such that $X_{n_{k(j)}}\to X$ almost 
    surely. By the Fatou's lemma, 
    \begin{equation*}
        \E\sbrc{X} = \E\sbrc{\liminf_{j\to\infty} X_{n_{k(j)}}} 
        \leq \liminf_{j\to\infty}\E\sbrc{X_{n_{k(j)}}} 
        = \lim_{k\to\infty}\E\sbrc{X_{n_k}} = \liminf_{n\to\infty}\E\sbrc{X_n}. 
    \end{equation*}
\end{proof}
...
\begin{proof}
    ...
    $F(t_{j+1}^-) - F(t_j) \leq \epsilon$ for all $j$. To see the existence of 
    such partition, put $t_0 = -\infty$ and let 
    \begin{equation*}
...
\begin{definition}
    Let $\F_n$ be a sequence of $\sigma$-algebras. The \textbf{tail $\sigma$-alegra} 
    $\T$ is defined as $\T = \cap_{n}\sigma(\F_n,\ldots)$. 
\end{definition}

\begin{theorem}[Kolmogorov Zero-One Law]
    Let $\F_i$ be independent $\sigma$-algebras, $\G_n = \sigma(\F_n,\ldots)$ and the 
    tail $\sigma$-algebra $\G_{\infty} = \cap_{n=1}^\infty\G_n$. Then $\G_\infty$ is 
    $\P$-trivial. 
\end{theorem}
\begin{proof}
...
\begin{theorem}[Uniform Law of Large Number]
    Let $Z_i\iidsim\mu_Z$ and $\Theta\subset\R^n$ be  
    compact. Suppose that $g(Z,\theta)$ is continuous in $\theta\in\Theta$ 
    $Z$-almost surely and there is $F(z)\in\L^1(\mu_Z)$ such that 
    $\abs{g(Z,\theta)}\leq F(Z)$ for all $\theta\in\Theta$. Then 
    \begin{thmenum}
        \item The mapping $\theta\mapsto\E\sbrc{g(Z,\theta)}$ is 
        continuous. 
        \item \begin{equation*}
            \sup_{\theta\in\Theta}\abs{\frac{1}{n}\sum_{i=1}^ng(Z_i,\theta) - \E\sbrc{g(Z,\theta)}}\to 0
        \end{equation*}
        almost surely. 
    \end{thmenum} 
\end{theorem}
\begin{proof}
    For almost every $Z$, $g(Z,\theta)$ is continuous in $\theta$. 
    Hence for every $\epsilon>0$, there is a $\delta$ such that 
    $\abs{g(Z,\theta') - g(Z,\theta)}<\epsilon$ if $\abs{\theta' - \theta}<\delta$. 
    Now 
    \begin{equation*}
        \abs{\E\sbrc{g(Z,\theta')} - \E\sbrc{g(Z,\theta)}} 
        \leq \E\sbrc{\abs{g(Z,\theta') - g(Z,\theta)}} < \epsilon. 
    \end{equation*}
    This proves (a). 

    For (b), we claim that $\frac{1}{n}\sum_{i=1}^nF(Z_i)<C(\set{Z_i})$ for 
    almost every $\set{Z_i}$. Suppose not, then there is a set 
    of $\set{Z_i}$ with positive probability such that there 
    is a subsequence $n_j$ with 
    \begin{equation*}
        \frac{1}{n_j}\sum_{i=1}^{n_j}F(Z_i)\to\infty.
    \end{equation*}
    Then this implies that 
    \begin{equation*}
        \E\sbrc{F(Z)} = \E\sbrc{\frac{1}{n_j}\sum_{i=1}^{n_j}F(Z_i)}\to\infty, 
    \end{equation*}
    a contradiction. Also, for almost every $\set{Z_i}$, 
    \begin{equation*}
        \abs{\frac{1}{n}\sum_{i=1}^ng(Z_i,\theta)}\leq \frac{1}{n}\sum_{i=1}^nF(Z_i) < C(\set{Z_i})
    \end{equation*}
    is bounded uniformly in $\theta$. 
    
    Next, we claim that for almost every $\set{Z_i}$, 
    $n^{-1}\sum_{i=1}^ng(Z_i,\theta)$ is equicontinuous. To see this, 
    notice that $g(Z_i,\theta)$ is uniformly continuous. Thus the 
    modulus of continuity 
    \begin{equation*}
        \omega(Z_i,\delta) = \sup_{\abs{\theta - \theta'}\leq\delta}\abs{g(Z_i,\theta) - g(Z_i,\theta')}\to 0
    \end{equation*}
    as $\delta\to 0$. Also, $\omega(Z_i,\delta)\leq 2F(Z_i)$. By the LDCT, 
    \begin{equation*}
        \lim_{\delta\to 0}\E\sbrc{\omega(Z_i,\delta)} = \E\sbrc{\lim_{\delta\to 0}\omega(Z_i)} = 0. 
    \end{equation*}
    Now 
    \begin{equation*}
        \sup_{\abs{\theta - \theta'}\leq \delta}\abs{\frac{1}{n}\sum_{i=1}^ng(Z_i,\theta) - \frac{1}{n}\sum_{i=1}^ng(Z_i,\theta')} 
        \leq \frac{1}{n}\sum_{i=1}^n\omega(Z_i,\delta)\to\E\sbrc{\omega(Z_i,\delta)}
    \end{equation*}
    by the strong law of large number. Hence for all $\epsilon>0$, 
    we can select $\delta$ such that $\E\sbrc{\omega(Z_i,\delta)}<\epsilon$ 
    and $N$ such that for all $n\geq N$, 
    \begin{equation*}
        \abs{\sup_{\abs{\theta - \theta'}\leq \delta}\abs{\frac{1}{n}\sum_{i=1}^ng(Z_i,\theta) - \frac{1}{n}\sum_{i=1}^ng(Z_i,\theta')} - \E\sbrc{\omega(Z_i,\delta)}} < \epsilon. 
    \end{equation*}
    The $\delta$ is fixed for all $n\geq N$ and since $g(Z_i,\theta)$ are unifomly 
    continuous for $i = 1,\ldots,N$, we see that $\frac{1}{n}\sum_{i=1}^ng(Z_i,\theta)$ 
    is equicontinuous for almost every $\set{Z_i}$. 

    Suppose that $n^{-1}\sum_{i=1}^ng(Z_i,\theta)$ does not converges 
    uniformly to $\E\sbrc{g(Z,\theta)}$. Then there is a subsequence 
    $n_j$ such that 
    \begin{equation*}
        \sup_{\theta\in\Theta}\abs{n_j^{-1}\sum_{i=1}^{n_j}g(Z_i,\theta) - \E\sbrc{g(Z,\theta)}} > \epsilon. 
    \end{equation*}
    for all $n_j$. By the Arzela-Ascoli theorem, there is a further 
    subsequence that converges uniformly. By the strong law of large number, 
    for given $\theta$, 
    \begin{equation*}
        n^{-1}\sum_{i=1}^ng(Z_i,\theta)\to \E\sbrc{g(Z,\theta)}
    \end{equation*}
    almost surely. Hence the further subsequence converges to $\E\sbrc{g(Z,\theta)}$, 
    posing a contradiction. Thus 
    \begin{equation*}
        n^{-1}\sum_{i=1}^ng(Z_i,\theta) \to \E\sbrc{g(Z,\theta)}
    \end{equation*}
    almost surely.
